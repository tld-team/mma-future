<?php
namespace MMAF\DataEngine\Services\Formula;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

final class FormulaV14 {
	public const VERSION = 'v1.4';

	public static function config(): array {
		return array(
			'formula_version' => self::VERSION,
			'eligibility'     => array(
				'max_age'                  => 34,
				'inactivity_months'        => 24,
				'max_pro_fights_count'     => 30,
				'min_scoring_bouts'        => 1,
				'required_is_rankable'     => 1,
				'required_rankability'     => 'rankable',
				'excluded_statuses'        => array( 'deleted_soft', 'hidden', 'merged', 'retired' ),
				'loss_limit_by_wins'       => array(
					'20' => 5,
					'15' => 4,
					'10' => 3,
					'5'  => 2,
				),
				'missing_birthdate_blocks' => true,
				'missing_last_fight_blocks'=> true,
			),
			'base_record'    => array(
				'win'        => 0.5,
				'loss'       => -1.5,
				'draw'       => 0.0,
				'no_contest' => 0.0,
			),
			'finishes'       => array(
				'ko_tko'     => 1.0,
				'submission' => 1.0,
				'decision'   => 0.0,
				'dq'         => 0.0,
				'unknown'    => 0.0,
			),
			'age_adjustment' => array(
				'under_25' => 1.0,
				'25_to_30' => 0.0,
				'31_plus'  => -1.0,
			),
			'opponent_differential_for_wins' => array(
				'od_lte_minus_10'       => -2.0,
				'od_minus_9_to_minus_1' => 0.0,
				'od_0_to_10'            => 1.0,
				'od_11_to_30'           => 3.0,
				'od_31_to_60'           => 5.0,
				'od_61_to_100'          => 7.0,
				'od_gt_100'             => 10.0,
				'missing'               => 0.0,
			),
			'loss_quality_penalty' => array(
				'od_gt_0'                 => 0.0,
				'od_0_to_minus_10'        => -2.0,
				'od_minus_11_to_minus_20' => -5.0,
				'od_lte_minus_21'         => -10.0,
				'missing'                 => 0.0,
			),
			'confidence' => array(
				'sample_curve_numerator'               => 120.0,
				'sample_curve_offset'                  => 2.0,
				'full_confidence_sample_size'           => 10,
				'missing_prefight_penalty_per_ratio'    => 20.0,
				'missing_method_penalty_per_ratio'      => 0.0,
				'neutral_performance_raw_score'         => 0.0,
			),
			'normalization' => array(
				// Frozen after P1/P99 calibration with an outward safety margin.
				'raw_min'           => -4.0,
				'raw_max'           => 4.0,
				'calibrated'        => true,
				'calibration_basis' => '2026-06-18 live-data draft; 121 eligible fighters; P1=-1.8, P99=1.8; expanded to preserve confidence headroom and avoid low-confidence 100 scores',
			),
			'tie_breakers'   => array(
				'raw_score_desc',
				'wins_desc',
				'finish_rate_desc',
				'confidence_desc',
				'age_asc',
				'last_fight_date_desc',
				'fighter_id_asc',
			),
		);
	}

	public function base_record_points( string $result ): float {
		$config = self::config();

		return (float) ( $config['base_record'][ $result ] ?? 0.0 );
	}

	public function finish_points( string $result, ?string $method_category ): float {
		if ( 'win' !== $result ) {
			return 0.0;
		}

		$method = $method_category ? $method_category : 'unknown';
		$config = self::config();

		return (float) ( $config['finishes'][ $method ] ?? 0.0 );
	}

	public function age_adjustment_points( ?int $age ): float {
		if ( null === $age ) {
			return 0.0;
		}

		if ( $age < 25 ) {
			return 1.0;
		}

		if ( $age <= 30 ) {
			return 0.0;
		}

		return -1.0;
	}

	public function opponent_differential_points_for_win( ?int $opponent_prefight_diff ): float {
		if ( null === $opponent_prefight_diff ) {
			return 0.0;
		}

		if ( $opponent_prefight_diff <= -10 ) {
			return -2.0;
		}

		if ( $opponent_prefight_diff <= -1 ) {
			return 0.0;
		}

		if ( $opponent_prefight_diff <= 10 ) {
			return 1.0;
		}

		if ( $opponent_prefight_diff <= 30 ) {
			return 3.0;
		}

		if ( $opponent_prefight_diff <= 60 ) {
			return 5.0;
		}

		if ( $opponent_prefight_diff <= 100 ) {
			return 7.0;
		}

		return 10.0;
	}

	public function loss_quality_penalty_points( ?int $opponent_prefight_diff ): float {
		if ( null === $opponent_prefight_diff ) {
			return 0.0;
		}

		if ( $opponent_prefight_diff > 0 ) {
			return 0.0;
		}

		if ( $opponent_prefight_diff >= -10 ) {
			return -2.0;
		}

		if ( $opponent_prefight_diff >= -20 ) {
			return -5.0;
		}

		return -10.0;
	}

	public function confidence_score( int $sample_size, int $missing_prefight_count, int $missing_method_count = 0 ): float {
		if ( $sample_size <= 0 ) {
			return 0.0;
		}

		$config = self::config();
		$confidence = (array) $config['confidence'];
		$sample_score = (float) $confidence['sample_curve_numerator'] * $sample_size / ( $sample_size + (float) $confidence['sample_curve_offset'] );
		$sample_score = $sample_size >= (int) $confidence['full_confidence_sample_size'] ? 100.0 : $sample_score;
		$missing_prefight_ratio = min( 1.0, max( 0.0, $missing_prefight_count / $sample_size ) );

		$sample_score -= $missing_prefight_ratio * (float) $confidence['missing_prefight_penalty_per_ratio'];

		return $this->clamp( $sample_score, 0.0, 100.0 );
	}

	public function adjusted_raw_score( float $performance_raw_score, float $confidence_score ): float {
		$config = self::config();
		$neutral = (float) $config['confidence']['neutral_performance_raw_score'];
		$factor = $this->clamp( $confidence_score, 0.0, 100.0 ) / 100.0;

		return $neutral + ( ( $performance_raw_score - $neutral ) * $factor );
	}

	public function normalized_score( float $adjusted_raw_score ): float {
		$config = self::config();
		$raw_min = (float) $config['normalization']['raw_min'];
		$raw_max = (float) $config['normalization']['raw_max'];

		if ( $raw_max <= $raw_min ) {
			return 0.0;
		}

		$normalized = ( ( $adjusted_raw_score - $raw_min ) / ( $raw_max - $raw_min ) ) * 100.0;

		return $this->clamp( $normalized, 0.0, 100.0 );
	}

	private function clamp( float $value, float $min, float $max ): float {
		return max( $min, min( $max, $value ) );
	}
}
